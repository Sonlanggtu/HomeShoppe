...
{
    public partial class tai_khoan_quan_tri : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            string lock_user_Admin = Request.QueryString["username"];
            ...
            {
                if (lock_user_Admin != null)
                {
                    int checkbox = 3;
                    var taikhoan = new TaiKhoanDAO();
                    var getcheck = taikhoan.ViewAccount(lock_user_Admin).KhoaTaiKhoan;
                    if (getcheck == true)
                    {
                        checkbox = 0;
                    }
                    ...
                    lock_user_Admin_func(lock_user_Admin, checkbox);
                }

                ConnectString ConnectionString = new ConnectString();
                SqlConnection Conn = new SqlConnection(ConnectionString.ConnectionString());
                Conn.Open();
                string get_TaiKhoanQuanTri = "select TenTaiKhoan, Email, SoLanDangNhapSai,NgayTaiKhoanHetHan, KhoaTaiKhoan, ChucVu from TaiKhoan where XacDinhQuanTri = 1";
                ...
                LtTaiKhoanQuanTri.Text = Read_TaiKhoanQuanTri;
                Conn.Close();
            }
            
        }
        public void lock_user_Admin_func(string lock_user_client, int checkbox)
        {
            ConnectString ConnectionString = new ConnectString();
            SqlConnection Conn = new SqlConnection(ConnectionString.ConnectionString());
            Conn.Open();
